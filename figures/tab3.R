...
#  filter(time == "baseline" & acute == "rest") |>
  group_by(supplement, measure, time) |>
  ...
  mutate(supplement = if_else(supplement == "glucose",
                       ...
                       if_else(supplement == "placebo",
                               "Placebo", supplement))) |>
  mutate(measure = if_else(measure == "isok.240",
                           "240",
                           if_else(measure == "isok.60",
                                   "60",
                                   if_else(measure == "isom",
                                           "0", measure)))) |>
...
humac.flex <- tabdat |>
  ...
  compose(i = 1, j = c(1, 2, 3, 4, 5, 6, 7, 8, 9), part = "header",
          value = c(as_paragraph("Condition"),
                    ...
                    as_paragraph("23h after 2RT"),
                    as_paragraph("23h after 4RT"),
                    as_paragraph("23h after 5RT"),
                    as_paragraph("30min after 6RT"),
                    as_paragraph("2h after 6RT"),
                    as_paragraph("23h after 6RT"))) 
