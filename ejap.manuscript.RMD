...
*Results:* Between conditions, there were no significant differences in increases of total RNA and ribosomal RNA (mean difference `r total_rna_stats`, 47S, `r stat_rrna$trg_47s`; 18S, `r stat_rrna$trg_18s`; 28S, `r stat_rrna$trg_28s`; 5.8S, `r stat_rrna$trg_5.8s`; 5S, `r stat_rrna$trg_5s`), or levels of cellular myelocutmatosis oncogene, upstream binding factor and ribosomal protein S6 (*p* = `r min(p_west)`-`r max(p_west)`). There was a linear relationship between upstream binding factor and total RNA, where an increase of 1 standard deviance unit of upstream binding factor equated to a $\sim$ `r corr_incr`% increase in total RNA (*p* = `r corrp`). 
...
This study was financed through research funds from the Inland Norway University of Applied Sciences. There were no economical vested interest and everyone participating as researchers and associates, received only a regular salary during this study.

## Acknowledgements
Great thanks are in order to Henriette Spilhaug Bollandsås, Nora Lerdalen Bonsak and Thea Bøhn Nyløkken for their impressive commitment and dedication in the training intervention
the Inland Norway University of Applied Sciences, for funding this research. Gratitude is also due to Daniel Hammarström, Håvard Hamarsland and Prof. Stian Ellefsen for the guidance and <!-- needs more work -->
...
Conceived and designed research: SE, HH, DH.
Performed experiments: KL, DH, HH, HN, SCM.
Interpreted results of experiments: KL, DH.
Prepared Figures: KL, DH.
Drafted manuscripts: KL, DH.
Edited and revised manuscript: SE, HH, DH.
...
dxa.flex <- dxa.tab %>%
  ...
  set_caption(caption = "Table 1. Participant characteristics",
              align_with_table = FALSE)
...
primer.tab <- tab.dat %>%
  ...
  compose(i = 1, j = c(1, 2, 3, 4), part = "header",
          value = c(as_paragraph("Gene"),
                    ...
                    as_paragraph("E"))) %>%
  set_caption(caption = "Table 2. Primer Sequences",
              align_with_table = FALSE) %>%
  set_table_properties(layout = "autofit")
