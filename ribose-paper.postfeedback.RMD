...
stat_fun_rna <- function(model) {
  ...
  ci <- confint(model)
  meandiff <- round(100 * (exp(coefs[3,1]) -1), 1)
  cilower <- round(100 * (exp(ci[5,1]) -1), 1)
  ciupper <- round(100 * (exp(ci[5,2]) -1), 1)
  pval <- round(coefs[3,5], 3)
...
From baseline to post-intervention total RNA and rRNA increased on average by ~24% and ~33-43%, respectively (Figure 2). However, RT with glucose did not induce increased accumulation of total RNA (Figure 2A, mean difference `r total_rna_stats`) or rRNA  (Figure 2B; 47S, `r stat_rrna[[5]]`; 18S, `r stat_rrna[[1]]`; 28S, `r stat_rrna[[2]]`; 5.8S, `r stat_rrna[[3]]`; 5S, `r stat_rrna[[4]]`) compared to RT with placebo.
...
Levels of c-Myc increased by 42% more after RT with placebo compared to RT with glucose, a significant difference (Figure 3A: *p* = `r cdiff`). Levels of UBF and rpS6 increased by 19 and 23% more in placebo compared to glucose, with no significant differences between groups (Figure 3A: UBF: *p* = `r udiff`, rpS6: *p* = `r rpdiff`). The increase in both RT with glucose and placebo for all measured proteins was significant at post compared to baseline (*p* < 0.05). Further, we found a linear relationship between UBF and total RNA, where an increase of 1 SD unit of UBF equated to a 13% increase in total RNA (*p* = `r corrp`).
